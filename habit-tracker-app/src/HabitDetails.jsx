...
function CheckBoxDay({ name, setDaysSelected, daysSelected, updateGoalField }) {
  ...
  const handleToggle = () => {
    ...
    if (nextCheckedState) {
      ...
      updateGoalField(
        "daysSelected",
        daysSelected.filter((day) => day !== name),
      );
    }
  };
...
function GoalInfo({ habit, updateGoalField, setDisabled }) {
  ...
  const [taskDaysSelected, setTaskDaysSelected] = useState(
  ...
  if (firstItem && typeof firstItem === "object" && "seconds" in firstItem) {
    const formattedDates = daysInMonthSelected.map((timestamp) =>
      timestamp.toDate(),
    );
    ...
    <div id="goal-info">
      ...
      <div id="task-days">
        ...
        <select
          ...
          onChange={(e) => {
            updateGoalField("taskDays", e.target.value);
...
function Line({ isItHidden }) {
  return <div className="line" hidden={isItHidden} />;
}
...
function HabitDetails({ habit, uid, onClose, loadHabits }) {
  ...
  return (
    <div id="details-frame">
      ...
      <HabitType habit={editedHabit} updateHabitField={updateHabitField} />
      ...
      <Priority habit={editedHabit} updateHabitField={updateHabitField} />
